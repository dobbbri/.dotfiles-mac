--╔══════════════════════════════════════════════════════════════════════╗
--║  init-com-dap.lua — Mesma config do init-com-lazy.lua, com nvim-dap   ║
--║  adicionado pra debug de verdade (breakpoints, step, variáveis) em    ║
--║  JS/TS/Node — a maior lacuna funcional que a config tinha até aqui.   ║
...
require("lazy").setup({
  ...
  -- "js-debug-adapter" (debirado do vscode-js-debug, o mesmo motor que o
  ...
  ---------------------------------------------------------------------
  {
    ...
    dependencies = {
      "williamboman/mason.nvim",
      {
        ...
        opts = {
          ...
          -- não deixa o mason-nvim-dap registrar automaticamente; a gente
          -- define as "configurations" (launch/attach) na mão mais abaixo,
          -- então só queremos que ele cuide da instalação + do "adapters".
    ...
    config = function()
      ...
      -- Astro: o servidor de dev roda em Node por baixo do Vite, então
      -- debug de código server-side (endpoints, middleware, .ts) funciona
      -- via as mesmas configs acima; breakpoint DENTRO de um .astro em si
      -- não é suportado pelo js-debug-adapter (ele debuga JS/TS, não o
      -- template compilado do Astro).
    end,
  },
